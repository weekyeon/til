...
* 객체지향 프로그래밍에서는 객체의 무결성이 깨지는 것을 막기 위해 메소드를 통해 데이터를 변경하는 방법 선호
  * 예를 들어 자동차의 속도는 음수가 될 수 없는데, 외부에서 음수로 변경한 경우 객체의 무결성이 깨졌다고 함
  * 즉, 객체 외부에서 직접적으로 객체 데이터에 접근해 데이터를 마음대로 읽고 변경하면 무결성이 깨질 수 있음
* 클래스 선언 시 가능하다면 필드를 private로 선언해서 외부로부터 보호하고, 필드에 대한 Getter & Setter 메소드를 작성해서 필드값을 안전하게 변경하고 사용하는 것이 좋음
* Getter
  * 외부에서 객체 데이터를 읽을 때 사용하는 메소드
